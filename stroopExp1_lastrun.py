...
expName = 'stroopExp1'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\user\\Desktop\\StroopExp\\stroopExp1_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    Instructtext = visual.TextStim(win=win, name='Instructtext',
        text='Remember, choose the colour of the letter, ignoring the word.\n\nleft = green \nright = blue\nup = yellow\ndown = red',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if Instrtext.status == STARTED:
            ...
            if tThisFlipGlobal > Instrtext.tStartRefresh + 1-frameTolerance:
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    InstructComponents = [Instructtext, key_resp]
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if Instructtext.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
            # keep track of start time/frame for later
        ...
        # if Instructtext is stopping this frame...
        if Instructtext.status == STARTED:
            # is it time to stop? (based on local clock)
            if tThisFlip > 3-frameTolerance:
                # keep track of stop time/frame for later
                Instructtext.tStop = t  # not accounting for scr refresh
                Instructtext.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Instructtext.stopped')
                # update status
                Instructtext.status = FINISHED
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=['up','left','right','down'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if response.status == STARTED and not waitOnFlip:
                theseKeys = response.getKeys(keyList=['left','right','up','down'], ignoreKeys=["escape"], waitRelease=False)
    ...
    while continueRoutine:
        ...
        if textGoodbye.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
            # keep track of start time/frame for later
