...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\user\\Desktop\\Exp\\StroopExp\\stroopExp_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    welcome_screen_Components = [textWelcomeMessage, key_Welcome]
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        # if key_Welcome is stopping this frame...
        if key_Welcome.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_Welcome.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
                key_Welcome.tStop = t  # not accounting for scr refresh
                key_Welcome.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_Welcome.stopped')
                # update status
                key_Welcome.status = FINISHED
                key_Welcome.status = FINISHED
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('blank_500.started', globalClock.getTime())
    # keep track of which components have finished
    blank_500Components = [textBlank500]
    ...
    for thisComponent in blank_500Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('blank_500.stopped', globalClock.getTime())
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
    ...
    trials = data.TrialHandler(nReps=5.0, method='random', 
        ...
        trialList=data.importConditions('stroop.stimuli.xlsx'),
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('stroop_trial.started', globalClock.getTime())
        ...
        _keyStroop_allKeys = []
        # keep track of which components have finished
        stroop_trialComponents = [textStroopWord, keyStroop]
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            # if textStroopWord is starting this frame...
            if textStroopWord.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # update status
                textStroopWord.status = STARTED
                textStroopWord.setAutoDraw(True)
            
            # if textStroopWord is active this frame...
            if textStroopWord.status == STARTED:
                # update params
                pass
            
            # if textStroopWord is stopping this frame...
            if textStroopWord.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > textStroopWord.tStartRefresh + 20-frameTolerance:
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    textStroopWord.status = FINISHED
            ...
            if keyStroop.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > keyStroop.tStartRefresh + 20-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if keyStroop.status == STARTED and not waitOnFlip:
                theseKeys = keyStroop.getKeys(keyList=['r','g','b','y'], ignoreKeys=["escape"], waitRelease=False)
        ...
        for thisComponent in stroop_trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('stroop_trial.stopped', globalClock.getTime())
        # check responses
    ...
    # keep track of which components have finished
    Goodbye_ScreenComponents = [textGoodbye, key_goodbye]
    ...
    while continueRoutine:
        ...
        # if key_goodbye is starting this frame...
        ...
        if key_goodbye.status == STARTED and not waitOnFlip:
            theseKeys = key_goodbye.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if len(_key_goodbye_allKeys):
                ...
                key_goodbye.duration = _key_goodbye_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    if key_goodbye.keys != None:  # we had a response
        ...
        thisExp.addData('key_goodbye.duration', key_goodbye.duration)
    thisExp.nextEntry()
